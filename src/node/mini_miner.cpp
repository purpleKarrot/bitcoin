...
MiniMiner::MiniMiner(const CTxMemPool& mempool, const std::vector<COutPoint>& outpoints)
{
    ...
    for (const auto& outpoint : outpoints) {
        if (!mempool.exists(outpoint.GetTxid())) {
        ...
        m_requested_outpoints_by_txid[outpoint.GetTxid()].push_back(outpoint);
        ...
        std::set<MockEntryMap::iterator, IteratorComparator> ancestors;
        {
            ...
            while (!to_process.empty()) {
                ...
                for (const auto& input : (*iter)->second.GetTx().GetInputs()) {
                    if (auto parent_it{m_entries_by_txid.find(input.prevout.GetTxid())}; parent_it != m_entries_by_txid.end()) {
...
std::optional<CAmount> MiniMiner::CalculateTotalBumpFees(const CFeeRate& target_feerate)
{
    ...
    while (!to_process.empty()) {
        ...
        for (const auto& input : tx.GetInputs()) {
            if (auto parent_it{m_entries_by_txid.find(input.prevout.GetTxid())}; parent_it != m_entries_by_txid.end()) {
                if (!has_been_processed.contains(input.prevout.GetTxid())) {
                    to_process.insert(parent_it);
                }
