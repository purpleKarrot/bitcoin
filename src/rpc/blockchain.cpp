...
bool FindScriptPubKey(std::atomic<int>& scan_progress, const std::atomic<bool>& should_abort, int64_t& count, CCoinsViewCursor* cursor, const std::set<CScript>& needles, std::map<COutPoint, Coin>& out_results, std::function<void()>& interruption_point)
{
    ...
    while (cursor->Valid()) {
        ...
        if (count % 256 == 0) {
            ...
            uint32_t high = 0x100 * *UCharCast(key.txid().begin()) + *(UCharCast(key.txid().begin()) + 1);
        ...
        [](const RPCMethod& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    if (action == "status") {
        ...
        for (const auto& it : coins) {
            ...
            unspent.pushKV("txid", outpoint.txid().GetHex());
            unspent.pushKV("vout", outpoint.index());
        ...
        [](const RPCMethod& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    const auto AddSpend = [&](
        ...
        event.pushKV("prevout_txid", txin.prevout.txid().ToString());
        event.pushKV("prevout_vout", txin.prevout.index());
        ...
        for (const CTxMemPoolEntry& e : mempool.entryAll()) {
            ...
            for (size_t vin_idx = 0; vin_idx < tx->vin.size(); ++vin_idx) {
                ...
                if (!coin) {
                    ...
                    CTransactionRef prev_tx = CHECK_NONFATAL(mempool.get(txin.prevout.txid()));

                    if (txin.prevout.index() >= prev_tx->vout.size()) {
                        throw std::runtime_error("Invalid output index");
                    }
                    const CTxOut& out = prev_tx->vout[txin.prevout.index()];
