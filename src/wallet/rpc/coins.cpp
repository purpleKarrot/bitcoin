        ...
        [](const RPCMethod& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    for (unsigned int idx = 0; idx < output_params.size(); idx++) {
        ...
        const auto it = pwallet->mapWallet.find(outpt.txid());
        ...
        if (outpt.index() >= trans.tx->vout.size()) {
        ...
        [](const RPCMethod& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    for (const COutPoint& outpt : vOutpts) {
        ...
        o.pushKV("txid", outpt.txid().GetHex());
        o.pushKV("vout", outpt.index());
        ...
        [](const RPCMethod& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    for (const COutput& out : vecOutputs) {
        ...
        entry.pushKV("txid", out.outpoint.txid().GetHex());
        entry.pushKV("vout", out.outpoint.index());
        ...
        if (!out.depth) {
            ...
            pwallet->chain().getTransactionAncestry(out.outpoint.txid(), ancestor_count, unused_cluster_count, &ancestor_size, &ancestor_fees);
            if (ancestor_count) {
