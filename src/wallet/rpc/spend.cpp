            ...
            if (options.exists("inputs") && options.exists("send_max")) {
                ...
                for (const CTxIn& input : rawTx.vin) {
                    if (pwallet->IsSpent(input.prevout)) {
                        throw JSONRPCError(RPC_INVALID_PARAMETER, strprintf("Input not available. UTXO (%s:%d) was already spent.", input.prevout.txid().ToString(), input.prevout.index()));
                    }
                    const CWalletTx* tx{pwallet->GetWalletTx(input.prevout.txid())};
                    if (!tx || input.prevout.index() >= tx->tx->vout.size() || !pwallet->IsMine(tx->tx->vout[input.prevout.index()])) {
                        throw JSONRPCError(RPC_INVALID_PARAMETER, strprintf("Input not found. UTXO (%s:%d) is not part of wallet.", input.prevout.txid().ToString(), input.prevout.index()));
                    }
                    ...
                    total_input_value += tx->tx->vout[input.prevout.index()].nValue;
                }
            } else {
                ...
                for (const COutput& output : AvailableCoins(*pwallet, &coin_control, fee_rate, coins_params).All()) {
                    ...
                    CTxIn input(output.outpoint.txid(), output.outpoint.index(), CScript(), rbf ? MAX_BIP125_RBF_SEQUENCE : CTxIn::MAX_SEQUENCE_NONFINAL);
