...
WalletTx MakeWalletTx(CWallet& wallet, const CWalletTx& wtx)
{
    ...
    WalletTx result;
    result.tx = wtx.tx;
    result.txin_is_mine.reserve(wtx.tx->GetInputs().size());
    for (const auto& txin : wtx.tx->GetInputs()) {
        result.txin_is_mine.emplace_back(InputIsMine(wallet, txin));
    }
    result.txout_is_mine.reserve(wtx.tx->GetOutputs().size());
    result.txout_address.reserve(wtx.tx->GetOutputs().size());
    result.txout_address_is_mine.reserve(wtx.tx->GetOutputs().size());
    ...
    EXCLUSIVE_LOCKS_REQUIRED(wallet.cs_wallet)
{
    ...
    result.lock_time = wtx.tx->GetLockTime();
...
{
    WalletTxOut result;
    result.txout = wtx.tx->GetOutputs()[n];
    result.time = wtx.GetTxTime();
