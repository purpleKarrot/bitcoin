...
bool CWallet::IsMine(const COutPoint& outpoint) const
{
    ...
    if (outpoint.GetIndex() >= wtx->tx->vout.size()) {
        return false;
    }
    return IsMine(wtx->tx->vout[outpoint.GetIndex()]);
}
...
bool CWallet::SignTransaction(CMutableTransaction& tx) const
{
    ...
    for (auto& input : tx.vin) {
        ...
        if(mi == mapWallet.end() || input.prevout.GetIndex() >= mi->second.tx->vout.size()) {
            return false;
        }
        const CWalletTx& wtx = mi->second;
        int prev_height = wtx.state<TxStateConfirmed>() ? wtx.state<TxStateConfirmed>()->confirmed_block_height : 0;
        coins[input.prevout] = Coin(wtx.tx->vout[input.prevout.GetIndex()], prev_height, wtx.IsCoinBase());
    }
