...
CWalletTx* CWallet::AddToWallet(CTransactionRef tx, const TxState& state, const UpdateWalletTxFn& update_wtx, bool rescanning_old_block)
{
    ...
    if (IsWalletFlagSet(WALLET_FLAG_AVOID_REUSE)) {
        ...
        for (const CTxIn& txin : tx->GetInputs()) {
            ...
            SetSpentKeyState(batch, op.GetTxid(), op.GetIndex(), true, tx_destinations);
        }
            ...
            for (const CTxIn& txin : tx.GetInputs()) {
                ...
                while (range.first != range.second) {
                    if (range.first->second != tx.GetHash()) {
                        WalletLogPrintf("Transaction %s (in block %s) conflicts with wallet transaction %s (both spend %s:%i)\n", tx.GetHash().ToString(), conf->confirmed_block_hash.ToString(), range.first->second.ToString(), range.first->first.GetTxid().ToString(), range.first->first.GetIndex());
...
void CWallet::transactionAddedToMempool(const CTransactionRef& tx) {
    ...
    if (tx->GetVersion() == TRUC_VERSION) {
        ...
        for (const CTxIn& tx_in : tx->GetInputs()) {
            auto parent_it = mapWallet.find(tx_in.prevout.GetTxid());
...
{
    AssertLockHeld(cs_wallet);
    auto wtx = GetWalletTx(outpoint.GetTxid());
    if (!wtx) {
        return false;
    }
    if (outpoint.GetIndex() >= wtx->tx->GetOutputs().size()) {
        return false;
    }
    return IsMine(wtx->tx->GetOutputs()[outpoint.GetIndex()]);
}
...
bool CWallet::SignTransaction(CMutableTransaction& tx) const
{
    ...
    for (auto& input : tx.vin) {
        const auto mi = mapWallet.find(input.prevout.GetTxid());
        if(mi == mapWallet.end() || input.prevout.GetIndex() >= mi->second.tx->GetOutputs().size()) {
            return false;
        }
        const CWalletTx& wtx = mi->second;
        int prev_height = wtx.state<TxStateConfirmed>() ? wtx.state<TxStateConfirmed>()->confirmed_block_height : 0;
        coins[input.prevout] = Coin(wtx.tx->GetOutputs()[input.prevout.GetIndex()], prev_height, wtx.IsCoinBase());
    }
