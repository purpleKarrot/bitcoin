...
bool InputIsMine(const CWallet& wallet, const CTxIn& txin)
{
    ...
    if (prev && txin.prevout.GetIndex() < prev->tx->vout.size()) {
        return wallet.IsMine(prev->tx->vout[txin.prevout.GetIndex()]);
    }
    return false;
}
...
bool CachedTxIsTrusted(const CWallet& wallet, const CWalletTx& wtx, std::set<Txid>& trusted_parents)
{
    ...
    for (const CTxIn& txin : wtx.tx->vin)
    {
        ...
        const CTxOut& parentOut = parent->tx->vout[txin.prevout.GetIndex()];
        ...
        if (wtx.tx->vin.size() > 0)
        {
            ...
            for (const CTxIn& txin : wtx.tx->vin)
            {
                ...
                if(!InputIsMine(wallet, txin)) /* If this input isn't mine, ignore it */
                    continue;
                if(!ExtractDestination(wallet.mapWallet.at(txin.prevout.GetTxid()).tx->vout[txin.prevout.GetIndex()].scriptPubKey, address))
                    continue;
