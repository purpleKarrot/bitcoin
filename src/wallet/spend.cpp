...
TxSize CalculateMaximumSignedTxSize(const CTransaction &tx, const CWallet *wallet, const CCoinControl* coin_control)
{
    ...
    for (const CTxIn& input : tx.vin) {
        const auto mi = wallet->mapWallet.find(input.prevout.txid());
        // Can not estimate size without knowing the input details
        if (mi != wallet->mapWallet.end()) {
            assert(input.prevout.index() < mi->second.tx->vout.size());
            txouts.emplace_back(mi->second.tx->vout.at(input.prevout.index()));
                           ...
                           const CoinFilterParams& params)
{
    ...
    for (const auto& [outpoint, txo] : wallet.GetTXOs()) {
        ...
        if (tx_safe_cache.contains(outpoint.txid()) && !tx_safe_cache.at(outpoint.txid()).first) {
            continue;
        }
        ...
        if (!tx_safe_cache.contains(outpoint.txid())) {
            tx_safe_cache[outpoint.txid()] = {false, false};
            ...
            tx_safe_cache[outpoint.txid()] = {true, safeTx};
        }
        const auto& [tx_ok, tx_safe] = tx_safe_cache.at(outpoint.txid());
        if (!Assume(tx_ok)) {
            continue;
        }
    ...
    if (params.check_version_trucness && unconfirmed_truc_coins.size() > 0) {
        ...
        for (const auto& [type, output] : unconfirmed_truc_coins) {
            if (output.outpoint.txid() == truc_txid) {
...
{
    AssertLockHeld(wallet.cs_wallet);
    const CWalletTx* wtx{Assert(wallet.GetWalletTx(outpoint.txid()))};

    const CTransaction* ptx = wtx->tx.get();
    int n = outpoint.index();
    while (OutputIsChange(wallet, ptx->vout[n]) && ptx->vin.size() > 0) {
        const COutPoint& prevout = ptx->vin[0].prevout;
        const CWalletTx* it = wallet.GetWalletTx(prevout.txid());
        if (!it || it->tx->vout.size() <= prevout.index() ||
            !wallet.IsMine(it->tx->vout[prevout.index()])) {
            break;
        }
        ptx = it->tx.get();
        n = prevout.index();
    ...
    if (!coin_sel_params.m_avoid_partial_spends) {
        ...
        for (const auto& [type, outputs] : coins.coins) {
            for (const COutput& output : outputs) {
                ...
                wallet.chain().getTransactionAncestry(output.outpoint.txid(), ancestors, cluster_count);
