                ...
                EXCLUSIVE_LOCKS_REQUIRED(cs_main, pool.cs)
{
    ...
    for (const CTxIn& txin : tx.vin) {
        ...
        if (txFrom) {
            ...
            assert(txFrom->vout.size() > txin.prevout.GetIndex());
            assert(txFrom->vout[txin.prevout.GetIndex()] == coin.out);
        } else {
...
std::optional<std::pair<ScriptError, std::string>> CScriptCheck::operator()() {
    ...
    if (VerifyScript(scriptSig, m_tx_out.scriptPubKey, witness, m_flags, CachingTransactionSignatureChecker(ptxTo, nIn, m_tx_out.nValue, cacheStore, *m_signature_cache, *txdata), &error)) {
        ...
        auto debug_str = strprintf("input %i of %s (wtxid %s), spending %s:%i", nIn, ptxTo->GetHash().ToString(), ptxTo->GetWitnessHash().ToString(), ptxTo->vin[nIn].prevout.GetTxid().ToString(), ptxTo->vin[nIn].prevout.GetIndex());
