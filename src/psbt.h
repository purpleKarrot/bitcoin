...
public:
    ...
    inline void Unserialize(Stream& s) {
        ...
        while (!s.empty() && i < input_count) {
            if (psbt_ver < 2) {
                inputs.emplace_back(psbt_ver, tx->vin[i].prevout.txid(), tx->vin[i].prevout.index(), tx->vin[i].nSequence);
                s >> inputs.back();
            } else {
            ...
            if (input.non_witness_utxo) {
                if (psbt_ver < 2) {
                    if (input.non_witness_utxo->GetHash() != tx->vin[i].prevout.txid()) {
                        throw std::ios_base::failure("Non-witness UTXO does not match outpoint hash");
                    }
                    if (tx->vin[i].prevout.index() >= input.non_witness_utxo->vout.size()) {
