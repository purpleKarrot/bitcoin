...
CTxMemPool::setEntries CTxMemPool::CalculateMemPoolAncestors(const CTxMemPoolEntry &entry) const
{
    ...
    for (unsigned int i = 0; i < tx.vin.size(); i++) {
        std::optional<txiter> piter = GetIter(tx.vin[i].prevout.GetTxid());
        if (piter) {
    ...
    for (const auto& it : score_with_topo) {
        ...
        for (const CTxIn &txin : tx.vin) {
            ...
            if (it2 != mapTx.end()) {
                ...
                assert(tx2.vout.size() > txin.prevout.GetIndex() && !tx2.vout[txin.prevout.GetIndex()].IsNull());
...
{
    for (unsigned int i = 0; i < tx.vin.size(); i++)
        if (exists(tx.vin[i].prevout.GetTxid()))
            return false;
    return true;
}
...
std::optional<Coin> CCoinsViewMemPool::GetCoin(const COutPoint& outpoint) const
{
    ...
    if (ptx) {
        if (outpoint.GetIndex() < ptx->vout.size()) {
            Coin coin(ptx->vout[outpoint.GetIndex()], MEMPOOL_HEIGHT, false);
