...
BOOST_AUTO_TEST_CASE(test_big_witness_transaction)
{
    ...
    for(uint32_t i = 0; i < mtx.vin.size(); i++) {
        ...
        vChecks.emplace_back(coins[tx.GetInputs()[i].prevout.GetIndex()].out, tx, signature_cache, i, SCRIPT_VERIFY_P2SH | SCRIPT_VERIFY_WITNESS, false, &txdata);
...
BOOST_AUTO_TEST_CASE(spends_witness_prog)
{
    ...
    tx_spend.vin[0].prevout = COutPoint(tx_create.GetHash(), 0);
    AddCoins(coins, CTransaction{tx_create}, 0, false);
