        ...
        if (ptx_potential_parent) {
            ...
            for (const auto& child : orphanage->GetChildrenFromSamePeer(ptx_potential_parent, peer_id)) {
                assert(std::any_of(child->GetInputs().cbegin(), child->GetInputs().cend(), [&](const auto& input) {
                    return input.prevout.GetTxid() == ptx_potential_parent->GetHash();
                }));
            }
        }
            ...
            if (sim_announcements.size() < MAX_ANN && command-- == 0) {
                ...
                for (unsigned child_tx = 0; child_tx < NUM_TX; ++child_tx) {
                    ...
                    for (auto& txin : txn[child_tx]->GetInputs()) {
                        if (txin.prevout.GetTxid() == txn[tx]->GetHash()) {
                            child_of = true;
                            break;
                        }
                    }
            ...
            for (int phase = 0; phase < 2; ++phase) {
                ...
                for (auto& ann : sim_announcements) {
                    ...
                    for (const auto& vin : txn[ann.tx]->GetInputs()) {
                        if (vin.prevout.GetTxid() == txn[tx]->GetHash()) matching_parent = true;
                    }
