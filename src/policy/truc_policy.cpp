...
struct ParentInfo {
    ...
    ParentInfo() = delete;
    ParentInfo(const Txid& txid, const Wtxid& wtxid, std::uint32_t version, bool has_mempool_descendant) :
                                           ...
                                           const std::vector<CTxMemPoolEntry::CTxMemPoolEntryRef>& mempool_parents)
{
    ...
    if (ptx->GetVersion() == TRUC_VERSION) {
        ...
        if (has_parent) {
            ...
            for (const auto& package_tx : package) {
                ...
                for (auto& input : package_tx->GetInputs()) {
        ...
        for (const auto& index: in_package_parents) {
            if (package.at(index)->GetVersion() == TRUC_VERSION) {
                return strprintf("non-version=3 tx %s (wtxid=%s) cannot spend from version=3 tx %s (wtxid=%s)",
                                          ...
                                          int64_t vsize)
{
    ...
    if (ptx->GetVersion() != TRUC_VERSION) return std::nullopt;
